 ...
 *   2. Ubah bentuk datanya jadi persis format yang dibutuhin index.html
 *   3. Suntik data itu ke index.html (file yang sama, di folder yang sama),
 *      di antara penanda BUILD:RECIPE_DATA_START / BUILD:RECIPE_DATA_END
 *
 * File ini nulis ulang index.html DI TEMPAT — nggak bikin file/folder baru.
 * Aman dijalanin berkali-kali karena penandanya selalu ditaruh balik.
 *
 * Dijalankan otomatis oleh GitHub Actions (.github/workflows/deploy.yml),
 * tapi juga bisa dites manual di komputer sendiri:
 *
 *   GAS_URL="https://script.google.com/macros/s/XXXXX/exec" node build.js
 */
...
async function main() {
  ...
  const updated = injectRecipeData(html, recipeData);

  fs.writeFileSync(HTML_PATH, updated, 'utf8');

  console.log(`Selesai. index.html sudah diperbarui dengan data terbaru.`);
}
 ...
 * datanya tetap cocok sama kode di index.html (yang punya toggle ID/EN).
 * English-nya untuk sementara dup dari teks Indonesia (belum ada
...
function injectRecipeData(html, recipeDataObj) {
  const startIdx = html.indexOf(START_MARKER);
  const endIdx = html.indexOf(END_MARKER);

  if (startIdx === -1 || endIdx === -1) {
    throw new Error('Penanda BUILD:RECIPE_DATA tidak ditemukan di index.html');
  }
